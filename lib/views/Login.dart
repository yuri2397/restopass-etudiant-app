...
Future<AccessToken> loginRequest(String number, String password) async{
  ...
  try{
    ...
    if(response.statusCode == 200){
      ...
      return AccessToken(tokenType:"error", expiresIn: -1, accessToken: response.statusCode.toString(), refreshToken: response.statusCode.toString());
    }
  }
  catch(e){
    return AccessToken(tokenType:"error", expiresIn: -2, accessToken: "500", refreshToken: "500");
  }
}
...
class _LoginState extends State<Login> {
  ...
  void initState() {
    ...
    _image = Image.asset("assets/images/login.jpg", fit: BoxFit.cover,);
  }
                    ...
                    children: <Widget> [
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          errorText: _numberError ? "N° de dossier requi" : null,
                          prefixIcon: Icon(Icons.person_outline_sharp, color: kPrimaryColor,size: 30,),
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                            ...
                            errorText: _passwordError ? "Mot de passe requi" : null,
                            prefixIcon: Icon(Icons.lock_open, color: kPrimaryColor,size: 30,),
                            border: OutlineInputBorder()
                        ),
                        onChanged: (value){
                          _password = value;
  ...
  void _login(AccessToken accessToken){
    if(accessToken.expiresIn >= 0){
      ...
      if (accessToken.accessToken == "400" || accessToken.accessToken == "422"){
        _hasErrors = true;
        _message = "N° de dossier ou mot passe invalide";
      }
      else if(int.parse(accessToken.refreshToken) >= 500  || accessToken.accessToken == "400" || accessToken.accessToken == "404" ){
        _hasErrors = true;
